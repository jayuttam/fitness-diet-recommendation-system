...
export const getMLPrediction = async (req, res) => {
  try {
    ...
    console.log("Mapped activity level:", activityMap[activityLevel]);

    /* ===== CALL PYTHON ML API ===== */
    console.log("Calling Python ML API...");

    const response = await axios.post(
      "https://fitness-ml-service.onrender.com/predict",
      {
        age: parseInt(age),
        height_cm: parseFloat(height),
        weight_kg: parseFloat(weight),
        gender: gender === "other" ? "male" : gender, // safety fallback
    ...
    const validatedML = {
      ...
      diet_type:
        mlResult.diet_type !== undefined ? mlResult.diet_type : 1,
      workout_type:
        mlResult.workout_type !== undefined ? mlResult.workout_type : 1,
    ...
    console.log("Saved ML data for user:", userId);

    res.status(200).json({
      message: "ML prediction generated & saved successfully",
      ml: validatedML,
      mlResult: validatedML,
      success: true,
    });
    ...
    res.status(200).json({
      ...
      ml: fallbackML,
      mlResult: fallbackML,
      warning: "ML model failed, using fallback values",
    });
  }
};